...
Credentials come from a .env file in the project root, or from environment
variables if you would rather not use one. Set DB_PASSWORD before running:
...
def validate(row, ticker, trade_date, cur):
    ...
    if any(pd.isna(row[c]) for c in ("Open", "High", "Low", "Close", "Volume")):
        log_error(cur, ticker, trade_date, "null OHLCV value")
        return False
    if row["High"] < row["Low"]:
    ...
    for company_id, ticker in fetch_companies(cur):
        ...
        for idx, row in df.iterrows():
            ...
            batch.append((
                ...
                float(row["Close"]), float(adj_close),
                int(row["Volume"]),
            ))
