...
MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "models", "xgb_model.joblib")
...
def train_model(train_df):
    ...
    print("[4a] Computing scale_pos_weight from an 85/15 validation split of the training set ...")
    fit_df, _val_df = val_split(train_df)
    buy_count = int((fit_df["signal_type"] == "BUY").sum())
    sell_count = int((fit_df["signal_type"] == "SELL").sum())
    scale_pos_weight = sell_count / buy_count
    print(f"    85% slice class counts: BUY {buy_count:,}  SELL {sell_count:,}")
    print(f"    scale_pos_weight = {scale_pos_weight:.4f}")

    hyperparams = dict(BASE_HYPERPARAMS, scale_pos_weight=scale_pos_weight)

    # xgboost's sklearn API requires numeric class labels, unlike the other
    # models here which fit directly on the "BUY"/"SELL" strings.
    y_train = (train_df["signal_type"] == "BUY").astype(int)
...
def evaluate(model, test_df):
    ...
    y_pred = np.where(y_pred_num == 1, "BUY", "SELL")
...
def main():
    ...
    model, hyperparams = train_model(train_df)
